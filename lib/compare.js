(function() {
  ...
  Comparator.prototype._filterNodes = function(list) {
    ...
    for (i = 0, l = list.length; i < l; i++) {
      item = list.item(i);
      if (item.nodeType == type.COMMENT_NODE && !this._options.compareComments) continue;
      if (item.nodeType == type.TEXT_NODE && ('' + item.nodeValue).trim() == '') continue;
      ret.push(item);
    }
    return ret;
  };
  ...
  Comparator.prototype._compareAttributes = function(expected, actual) {
    ...
    for (i in aExpected) {
      ...
      if (aExpected.hasOwnProperty(i) && aActual.hasOwnProperty(i)) {
        ...
        if (vExpected !== vActual) {
          if (!this._collector.collectFailure(aExpected[i], aActual[i])) return false;
        }
    ...
    for (i in aExpected) {
      if (aExpected.hasOwnProperty(i)) if (!this._collector.collectFailure(aExpected[i], null)) return false;
    }

    // report all extra attributes
    for (i in aActual) {
      if (aActual.hasOwnProperty(i)) if (!this._collector.collectFailure(null, aActual[i])) return false;
    }
    ...
    if (left.nodeName === right.nodeName && left.nodeType == right.nodeType) {
      switch (left.nodeType) {
        ...
        case type.ELEMENT_NODE:
          return this._compareAttributes(left.attributes, right.attributes) && this._compareNodeList(left.childNodes, right.childNodes);
        case type.TEXT_NODE:
        // fallthrough
        ...
        case type.COMMENT_NODE:
          if (left.nodeType == type.COMMENT_NODE && !this._options.compareComments) return true;
