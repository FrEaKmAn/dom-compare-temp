(function() {
  ...
  Collector.prototype._describeNode = function(node) {
    if (node.nodeType == type.TEXT_NODE || node.nodeType == type.CDATA_SECTION_NODE || node.nodeType == type.COMMENT_NODE) {
      ...
      if (expected && !actual) {
        ...
        if (expected.nodeType == actual.nodeType) {
          if (expected.nodeName == actual.nodeName) {
            ...
            if (vExpected == vActual) throw new Error("Nodes are considered equal but shouldn't");
            else {
              diff.type = typeMap[expected.nodeType];
            ...
            diff.type = typeMap[expected.nodeType];
            msg = 'Expected ' + typeMap[expected.nodeType] + " '" + expected.nodeName + "' instead of '" + actual.nodeName + "'";
            canContinue = false;
          }
        } else {
          diff.desc = 'tag:changed';
